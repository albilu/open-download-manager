...
- **Step 5 (in progress)**: **all 8 glade files ported faithfully** (main-window 1:1 with menu-as-GMenu; new-download; settings 7-tab; property; about; start-shutdown; import-list; import-sequence) with `WindowSmokeTest` 8/8 green. Wired: full download/new-download/settings/property dialogs (with new manager-level `changeSettings()`), import list + URL sequence execution, DownloadSettingsFactory applying persisted engine defaults, Tor toggle, completion actions (notify/shutdown), website-scrape dialog, clipboard toggle, window-state save/restore. Remaining: tray/background, scheduler UI, queue reorder + live search (need core APIs), Trackers/Peers/Files tabs (need aria2 RPC), DHT/uploadSpeed, info-hash. `[LATER]` items skipped per README.
