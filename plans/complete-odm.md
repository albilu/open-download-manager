...
- *Step 4 rescope (in progress, commit `8d3a76e`)*: framework + 4 dialogs done (main window, new-download, settings, import-list, about). Signal connections are programmatic (not `.ui` handler names) and widget lookup is fail-fast via `Widgets.require`; `WindowSmokeTest` enforces `.ui`↔code integrity. `@GtkTemplate` dropped deliberately: programmatic connection gives the same compile-time safety without a gresource build pipeline. Deferred to Step 5: property/import-sequence/start-shutdown dialogs, menu structure, category filtering, torrent file list.
